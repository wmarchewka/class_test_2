...
class Loader(object):
    ...
    def __init__(self):
        ...
        switches_class = switches.Switches(spi_class,gains_class, gpio_class, coderategenerator_class, digitalpots_class)
        currentsense_class = currentsense.CurrentSense(spi_class)
        rotary_class = rotary.Rotary(pollingpermission_class, gpio_class, config_class, speedgenerator_class, digitalpots_class)
        gui_class = gui.Mainwindow(config_class, digitalpots_class, secuirtylevel_class, gpio_class)
        pollvalues_class = pollvalues.Pollvalues(pollingpermission_class, currentsense_class, switches_class, config_class)
